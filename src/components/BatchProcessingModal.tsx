...
import { X, Layers, Upload, Download, Check, Sparkles, Loader2, PenTool, Star, BookmarkCheck, Store } from "lucide-react";
              ...
              <div className="pt-0.5 space-y-1.5">
                {savedSignatures.length > 0 ? (
                  ...
                  <p className="text-[10px] text-amber-700 dark:text-amber-300 italic">
                    Aucune signature enregistrée dans "Mes Signatures".
                  </p>
                )}
                ...
                {onOpenKaboStore && (
                  <button
                    ...
                    className="w-full py-1 px-2 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 text-[11px] font-bold flex items-center justify-center space-x-1 transition cursor-pointer"
                  >
                    <Store className="w-3 h-3 text-amber-500" />
                    <span>Explorer / Récupérer depuis KABO Store</span>
                  </button>
                )}
              </div>
            )}
